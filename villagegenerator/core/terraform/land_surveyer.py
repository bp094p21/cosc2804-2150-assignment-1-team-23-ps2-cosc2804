...
class TerrainScaner():
    ...
    def true_scan_terrain(self, z_length, x_length, under_limit_num):  
        ...
        blocks = self.mc.getBlocks(x, 50, z, x+x_length-1, acceptable_height + 15, z+z_length-1) #0-256 is standard world height
        ...
        for row in height_array: # go through every value to see if its under the limit or not, if it is add 1 to under_limit
            for height in row:
                if height < limit:
                    under_limit += 1
        ...
        if under_limit <= under_limit_num:
            ...
            print(f'The limit is y={int(limit)} and a maximum of {under_limit_num} blocks can be under it. There were {under_limit} blocks under the limit.')
            return True
        else:
            print("This terrain is not suitable for village generation. Please find another area and try again.")
            print(f'The limit is y={int(limit)} and a maximum of {under_limit_num} blocks can be under it. There were {under_limit} blocks under the limit.')
            self.mc.postToChat("This terrain is not suitable for village generation. Please find another area and try again.")
            return False