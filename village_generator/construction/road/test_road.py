...
def get_lowest_y_in_strip(mc, x1, y1, z1, x2, z2):
    lowest = y1
...
#     mc.setBlocks(x1, y1 + 15, z1, x1, get_lowest_y_in_strip(mc, x1, y1, z1, x1, z2) + 1, z2, block.AIR)

# def flatten_ns(mc, x1, y1, z1, x2):
#     mc.setBlocks(x1, y1 + 15, z1, x2, get_lowest_y_in_strip(mc, x1, y1, z1, x2, z1) + 1, z1, block.AIR)


def flatten_strip(mc, x1, y1, z1, x2, z2):
    if x1 == x2:
        mc.setBlocks(x1, y1 + 20, z1, x1, get_lowest_y_in_strip(mc, x1, y1, z1, x1, z2) + 1, z2, block.AIR)
    elif z1 == z2:
        mc.setBlocks(x1, y1 + 20, z1, x2, get_lowest_y_in_strip(mc, x1, y1, z1, x2, z1) + 1, z1, block.AIR)
...
def build_straight_ew_terraform(mc, x, y, z):
    ...
    for i in range(15):
        flatten_strip(mc, x + i, mc.getHeight(x + i, start + z), start + z, x + i, start + z + 9)
...
def build_straight_ns_terraform(mc, x, y, z):
    ...
    # first, flatten all the path, so that it is valid to place roads upon
    for k in range(15):
        flatten_strip(mc, start + x, mc.getHeight(start + x, z + k), z + k, start + x + 9, z + k)
...
def build_straight_ew_terraform_skinny(mc, x, y, z):
    ...
    # first, flatten all the path, so that it is valid to place roads upon
    for i in range(15):
        flatten_strip(mc, x + i, mc.getHeight(x + i, start + z + 6), start + z + 6, x + i, start + z + 8) #starts at the 6th block in the 15 length to be centred in plot.

    # build the roads on the validated path.

    for i in range(15):
        new_y = mc.getHeight(x + i, start + z + 6)
        mc.setBlocks(x + i, new_y, start + z + 6, x + i, new_y, start + z + 8, block.BRICK_BLOCK)

        # Will create evenly placed glowstone blocks down the center of the path
        if i == 2:
            mc.setBlock(x + 2, new_y, z + 7 + start, block.GLOWSTONE_BLOCK)
        elif i == 7:
            mc.setBlock(x + 7, new_y, z + 7 + start, block.GLOWSTONE_BLOCK)
        elif i == 12:
            mc.setBlock(x + 12, new_y, z + 7 + start, block.GLOWSTONE_BLOCK)


# This function will ensure that the level of elevation for each y-level increase is at most 1 block, so that the player can jump and does not have to destroy/place blocks (or fly) to traverse the path.
# It works by obtaining the gradient that two 3D Vectors make with the y-axis (so the j component of the vector). We then get this gradient as a scalar, and if it is above 0.5, then we
# keep adding 1 to the z coordinate so that it pushes the blueprint out by one, until the gradient is satisfactory, then we can build up one block at a time and increment the z one at a time.
# that way it forms a 1-block max elevated staircase to the top (or bottom) of the mountain and you can walk up it. It is basically like a blueprint function that goes over something until its right
# then finally builds it. 0.5 has significance, because it allows us to be able to actually build with a 1-block elevation max, so once that is satisfied, we can just blindly build the staircase.
def enforce_max_steepness():
    pass
...
def build_straight_ns(mc, x, y, z):
    ...
    for k in range(15):
        mc.setBlock(start + x, mc.getHeight(start + x, z + k), z + k, wood_id)
        mc.setBlock(start + x, mc.getHeight(start + x, z + k) + 1, z + k, block.LEAVES)
        mc.setBlock(start + x + 1,  mc.getHeight(start + x + 1, z + k), z + k, wood_id, 4)
        
        mc.setBlock(start + x + 2,  mc.getHeight(start + x + 2, z + k), z + k, block.BRICK_BLOCK)
        mc.setBlock(start + x + 3,  mc.getHeight(start + x + 3, z + k), z + k, block.BRICK_BLOCK)
        mc.setBlock(start + x + 4,  mc.getHeight(start + x + 4, z + k), z + k, block.BRICK_BLOCK)
        mc.setBlock(start + x + 5,  mc.getHeight(start + x + 5, z + k), z + k, block.BRICK_BLOCK)
        mc.setBlock(start + x + 6,  mc.getHeight(start + x + 6, z + k), z + k, block.BRICK_BLOCK)

        mc.setBlock(start + x + 7,  mc.getHeight(start + x + 7, z + k), z + k, wood_id, 4)
        temp = mc.getHeight(start + x + 8, z + k)
        mc.setBlock(start + x + 8, temp + 1, z + k, block.LEAVES)
        mc.setBlock(start + x + 8, temp, z + k, wood_id)


    mc.setBlock(x + 4 + start, mc.getHeight(x + 4 + start, z + 2), z + 2, block.GLOWSTONE_BLOCK)
    mc.setBlock(x + 4 + start, mc.getHeight(x + 4 + start, z + 7), z + 7, block.GLOWSTONE_BLOCK)
    mc.setBlock(x + 4 + start, mc.getHeight(x + 4 + start, z + 2), z + 12, block.GLOWSTONE_BLOCK)
if __name__ == '__main__':
    ...
    #for i in range(15):
    # build_straight_ew_terraform_skinny(mc, x, y, z)
    # build_straight_ew_terraform_skinny(mc, x + 15, y, z)
    # build_straight_ew_terraform_skinny(mc, x + 30, y, z)

    #build_straight_ew_replace(mc, x, y, z + 12)
    build_straight_ns(mc, x, y, z)
