...
def get_layout(
    ...
    """Takes house type in string and plot length in int to return a object with a dictionary of layouts for various components of a property."""
    if house_type == 'basic':
...
class Layout:
    """Upon init, returns an object with a dictionary of layouts for property components and some of the house components. It takes a bird's eye view approach for each layout. z_offset represent the offset from the property's entrance_edge; its axis perpendicular to it.\nx_offset represents the offset in the axis parallel to the entrance_edge.\nThe layouts do not consider ABSOLUTE orientation, rather, the relative orientations of each component to each other. The component designer found in designer.py uses the layout for each component, along with the absolute orientation of the property to properly position and orientate each component."""
    name = None
    emoji = '📍'
    plot_length = None
    layout = {}         
